...
# had to add lines 5-6, to get matplotlib to work in my virtual environment
import matplotlib 
...
X_C = 0.7 # 0.7 is a filler value to run the code with
...
#epsilon values of other main sequences (Helium, Carbon)
def eps_he(rho, T):
    return (3.85E-8) * (Y ** 3) * ((rho * 1E-5) ** 2) * ((T * 1E-8)**44)

def eps_cc(rho, T):
    return (5E4) * (X_C ** 2) * (rho * 1E-5) * ((T * 1E-9) **30)
...
eps_0_he = eps_he(rho_c, T_c)
eps_0_cc = eps_cc(rho_c, T_c)
M = (4 * math.pi / 3.0) * (r_0 ** 3) * rho_c
lum = (4 * math.pi / 3.0) * (r_0 ** 3) * rho_c * eps_0
lum_he = (4 * math.pi / 3.0) * (r_0 ** 3) * rho_c * eps_0_he
lum_cc = (4 * math.pi / 3.0) * (r_0 ** 3) * rho_c * eps_0_cc
...
for rad in np.linspace(0.01, r_surf, 100000):
    """This loops over a range of radius values at a  specified step size
    ...
    n0_he = h * dL_dR(rho, rad, epshe)
    n0_cc = h * dL_dR(rho, rad, epscc)
    ...
    n1_he = h * dL_dR(rho + 0.5 * k0, rad + 0.5 * h, epshe)
    n1_cc = h * dL_dR(rho + 0.5 * k0, rad + 0.5 * h, epscc)
    ...
    n2_he = h * dL_dR(rho + 0.5 * k1, rad + 0.5 * h, epshe)
    n2_cc = h * dL_dR(rho + 0.5 * k1, rad + 0.5 * h, epscc)
    ...
    n3_he = h * dL_dR(rho + k2, rad + h, epshe)
    n3_cc = h * dL_dR(rho + k2, rad + h, epscc)
    ...
    # new helium luminosity
    lum_he += (1 / 6.0) * (n0_he + 2 * n1_he + 2 * n2_he + n3_he)
    # new carbon luminosity
    lum_cc += (1 / 6.0) * (n0_cc + 2 * n1_cc + 2 * n2_cc + n3_cc)
    # new tau
    ...
    T_vals.append(T)
    L_vals.append(lum)
    L_he_vals.append(lum_he)
    L_cc_vals.append(lum_cc)
...
print("Helium Luminosity: {}".format(lum_he))
print("Carbon Luminosity: {}".format(lum_cc))
...
fig5 = plt.figure()
plt.plot(np.linspace(0.01, r_surf, 100000), L_he_vals)
plt.title("Helium Luminosity vs. Radius")
plt.xlabel("Radius")
plt.ylabel("Luminosity")
# fig5.show()

fig6 = plt.figure()
plt.plot(np.linspace(0.01, r_surf, 100000), L_cc_vals)
plt.title("Carbon Luminosity vs. Radius")
plt.xlabel("Radius")
plt.ylabel("Luminosity")
# fig6.show()
plt.show()
